...
- Use `pushd` to switch folder so that `popd` will switch back.
- Support URL with `st cs`.
...
See [HACKING.md](HACKING.md) for `npm publish` instructions.

## Testing

Ensure the full functionality of changing to a project directory works.

Ensure that uninstalling with `npm uninstall -g` cleans up the bin file (steacc\_, steacc, st)
...
_This section was written by Copilot._
